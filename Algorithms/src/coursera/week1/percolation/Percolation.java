...
public class Percolation {
	...
	// number of grid should be n +1，because row and column indices are between 1
	// and n
	private int n;
	 ...
	 *
	 * @param i1
	 *            adjacent open site row index
	 * @param j1
	 *            adjacent open site col index
	 * @param i2
	 *            the opened site row index
	 * @param j2
	 *            the opened site col index
	 */
	private void update(int i1, int j1, int i2, int j2) {
		int p = uf.find(i1 * n + j1);
		int q = uf.find(i2 * n + j2);
		uf.union(i1 * n + j1, i2 * n + j2);
		// if one of them is connected to bottom, then the updated component is
		// connected to bottom too.
